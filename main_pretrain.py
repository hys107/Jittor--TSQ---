...
def main():
    ...
    fix_random_seeds(args.seed)  #设置随机种子
    logger, training_stats = initialize_exp(args, "epoch", "loss")   #初始化参数
    ...
    logger.info("Building data done with {} images loaded.".format(len(train_dataset)))#配置日志文件地址
    ...
    if jt.in_mpi:
        for n, p in model.named_parameters():  #迭代打印
            p.assign(p.mpi_broadcast())  #将一个数据广播到其他进程相同buffer变量中

    # copy model to GPU  将模型复制到GPU
    if jt.rank == 0:
        logger.info(model)
    ...
    # optionally resume from a checkpoint   监测点恢复
    to_restore = {"epoch": 0}
    restart_from_checkpoint(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train the network for one epoch   训练网络一次
        logger.info("============ Starting epoch %i ... ============" % epoch)

        # optionally starts a queue   启动阵列
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, (inputs, gridq, gridk) in enumerate(train_loader):
        ...
        # ============ backward and optim step ... ============ 向后优化步骤
...
def swav_loss(args, model, embedding, output, queue, use_the_queue, bs):
    ...
    for i, crop_id in enumerate(args.crops_for_assign):
        with jt.no_grad():
            ...
            # get assignments  获得任务
            q = distributed_sinkhorn(args, out)[-bs:]
            labels.append(q)

        # cluster assignment prediction  聚类分配预测
        subloss = 0
...
def d2s_loss(args, output, labels, bs, shallow=None):
    ...
    # alignment from deep to shallow  对齐浅层与深层
    losses_shallow = []
